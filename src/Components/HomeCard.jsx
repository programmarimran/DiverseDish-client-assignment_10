...
const HomeCard = ({recipe}) => {
  ...
  return (
    <div className="bg-gradient-to-b from-[#f0f4ff] to-[#dbeafe] mx-auto w-full h-full rounded-2xl shadow-md hover:shadow-lg transition-shadow duration-300">
      <img
        src={recipe?.image}
        alt={recipe?.title}
      ...
      <div className="p-4 space-y-2 shadow-2xl border-t-2 border-gray-200 rounded-b-2xl">
        <div className="w-11/12 space-y-4 mx-auto">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-amber-400 text-sm p-1 rounded-xl">{recipe?.cuisineType}</div>
          </div>
          ...
          <div className="flex items-center justify-between text-sm mt-2">
            <div className="text-yellow-500 font-bold">❤️ {recipe?.likeCount} Likes</div>
            <div className="text-gray-500">{recipe?.categories.join(", ")}</div>
          </div>

          <button className="btn btn-primary btn-sm w-full mt-4">View Details</button>
        </div>
      </div>
    </div>
  );
